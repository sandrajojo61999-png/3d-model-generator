...
export default function Home() {
    ...
    <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="bg-gray-900 rounded-2xl p-6">
        ...
        {image && <div>
          ...
          <button onClick={refine} disabled={loading || !refineText} className="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-gray-600 rounded-xl py-3 font-semibold transition mb-3">Refine Model</button>
          <a href={`${API}/download/stl`} download="model.stl" className="block text-center w-full bg-green-600 hover:bg-green-700 rounded-xl py-3 font-semibold transition">Download STL</a>
        </div>}
    ...
    <div className="max-w-6xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-gray-900 rounded-2xl p-6">
          ...
          {image && <a href={`${API}/download/stl`} download="model.stl" className="block text-center w-full bg-green-600 hover:bg-green-700 rounded-xl py-3 font-semibold transition mt-3">Download STL</a>}
          {status && <div className="mt-4 bg-blue-900 rounded-xl p-3 text-sm text-blue-200">{status}</div>}
          {error && <div className="mt-4 bg-red-900 rounded-xl p-3 text-sm text-red-200">{error}</div>}
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-6">
      ...
      {mode === 'openscad' ? <TextMode /> : <ImageMode />}
    </main>
  );
}
